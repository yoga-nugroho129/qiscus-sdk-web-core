...
export default class MqttAdapter {
  constructor (url, callbacks, context) {
    ...
    if (this.context.mqttURL) {
      url = this.context.mqttURL
    }
    ...
    this.mqtt.on('message', (topic, message) => {
      ...
      if (topic.length == 2) {
        ...
        if (self.context.selected && self.context.singleRoomSubscribe) {
          ...
          if (!self.context.typingStatus[topic[1]]) {
            self.context.typingStatus[topic[1]] = ''
          }
          if (topic[3] != self.context.user_id) {
            // Emit typing event
            self.context.emit('typing', {
            ...
            if (message == '1' && topic[3] != self.context.user_id) {
              self.context.typingStatus[topic[1]] = `${participant} is typing ...`
            }
            if (message == '0') {
              self.context.typingStatus[topic[1]] = ''
            }
        ...
        if (room) {
          ...
          if (topic[3] == commentToFind.username_real) {
            return false
          }
          const options = {
          ...
          if (!commentToFind.room_id) {
            commentToFind.room_id = room.id
          }
        ...
        if (room) {
          ...
          if (!commentToFind || commentToFind.isDelivered || commentToFind.isRead) {
            return false
          }
          // Debugger;
          if (topic[3] == commentToFind.username_real) {
