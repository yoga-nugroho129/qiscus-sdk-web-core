...
class QiscusSDK extends EventEmitter {
  ...
  init (config) {
    ...
    if (config.singleRoomSubscribe) {
      this.singleRoomSubscribe = config.singleRoomSubscribe
    }
  ...
  setEventListeners () {
    ...
    self.on('newmessages', function (comments) {
      ...
      comments.forEach(comment => {
        ...
        const isAlreadyRead =
          comment.id <= self.last_received_comment_id
        if (!room) {
          if (!isAlreadyRead) {
     ...
     */
    self.on('comment-delivered', response => {
     ...
     */
    self.on('chat-room-created', response => {
     ...
     */
    self.on('group-room-created', response => {
      self.isLoading = false
     ...
     */
    self.on('header-clicked', response => {
  ...
  setUserWithIdentityToken (data) {
    if (!data || !('user' in data)) {
      return this.emit('login-error', data)
    }
  ...
  synchronize (last_id) {
    ...
    this.userAdapter.sync(idToBeSynced).then(comments => {
      ...
      if (comments.length > 0) {
        this.emit('newmessages', comments)
      }
    })
  }
  ...
  getRoomById (id) {
    ...
    return self.roomAdapter.getRoomById(id).then(
      response => {
        ...
        let room
        const roomToFind = self.rooms.find(r => r.id == id)
  ...
  getOrCreateRoomByUniqueId (id, room_name, avatar_url) {
    ...
    return self.roomAdapter.getOrCreateRoomByUniqueId(id, room_name, avatar_url)
      .then(response => {
        // Make sure the room hasn't been pushed yet
        let room
        const roomToFind = self.rooms.find(room => {
          id
        })
        if (!roomToFind) {
          room = new Room(response)
  ...
  sendComment (
    ...
    const pendingCommentDate = new Date()
    const commentData = {
    ...
    if (type == 'reply') {
      ...
      const replied_message = self.selected.comments.find(cmt => cmt.id == parsedPayload.replied_comment_id)
  ...
  removeSelectedRoomParticipants (values = [], payload = 'id') {
    if (!values) {
      return Promise.reject({ message: 'Please gives an array values.' })
    }
    const participants = this.selected.participants
    if (!participants) {
      return Promise.reject({ message: 'Nothing selected room chat.' })
    }
  ...
  publishEvent (roomId, payload) {
    ...
    if (typeof roomid !== 'string') {
      throw new TypeError('`roomId` must be type of string')
    }
